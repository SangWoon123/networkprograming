...
public class SignUpService {
    ...
    public boolean signUp(String username, String password, Socket d_socket) {
        ...
        if (userExists(username,d_socket)) {
            ...
            sendWriter.println("이미 등록된 사용자 이름입니다.");
            sendWriter.flush();
            return false;
        }
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(USER_DATA_FILE, true))) {
            ...
            sendWriter.println("파일에 데이터를 쓰는 중 오류가 발생했습니다.");
            sendWriter.flush();
            return false;
        }

        System.out.println("회원가입이 완료되었습니다.");
        sendWriter.println("회원가입이 완료되었습니다.");
        sendWriter.flush();
        return true;
    }
    ...
    private boolean userExists(String username,Socket d_socket) {
        ...
        try (Scanner scanner = new Scanner(new File(USER_DATA_FILE))) {
            ...
            sendWriter.println("파일에서 데이터를 읽는 중 오류가 발생했습니다.");
            sendWriter.flush();
        }
